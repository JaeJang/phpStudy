...
Question.prototype = 
{
	...
	createElements : function()
	{
		...
		for(let i = 0; i < 3; ++i)
		{
			let input = document.createElement('input');
			input.type = 'radio';
			input.name = "difficulty"+this.num;
			this.diffi.push(input);
		}
		this.diffi[0].value = "Hard";
		this.diffi[1].value = "Medium";
		this.diffi[2].value = "Easy";

		let label_hard = document.createElement('label');
		let label_medium = document.createElement('label');
		let label_easy = document.createElement('label');
		label_hard.innerHTML = "hard";
		label_medium.innerHTML = "medium";
		label_easy.innerHTML = "easy";
		this.questionDiv.appendChild(this.diffi[0]);
		this.questionDiv.appendChild(label_hard);
		this.questionDiv.appendChild(this.diffi[1]);
		this.questionDiv.appendChild(label_medium);
		this.questionDiv.appendChild(this.diffi[2]);
		this.questionDiv.appendChild(label_easy);
	...
	checkBlanks: function()
	{
		...
		let checkDifficulty = false;
		for(let i = 0; i < this.diffi.length; ++i)
		{
			if(this.diffi[i].checked ===true)
				checkDifficulty = true;
		}
		if(!checkDifficulty)
			errMessage.push("One difficulty must be selected");
		
		if(check && checkCheckboxes && checkDifficulty)
		{
			this.removeErrorMes();
			return true;
		}
	...
	setNum: function(num)
	{
		...
		for(let i = 0; i < this.diffi.length; ++i)
		{
			this.diffi[i].name = "difficulty"+this.num;
		}
	},
